...
impl Approved {
        ...
        match msg.variant() {
            ...
            Variant::UserMessage(content) => {
                if msg.dst() == &DstLocation::AccumulatingNode(self.node().name()) {
                    use crate::messages::SrcAuthority;
                    if let SrcAuthority::Partial { proof_share, .. } = msg.src() {
                        match self
                            .message_accumulator
                            .add(&msg.signing_bytes()?, proof_share.clone())
                        {
                            Ok(_) => {
                            ...
                            Err(err) => {
                                warn!("Error accumulating message at destination: {:?}", err);
                                Ok(vec![])
                            }
                        }
                    } else {
    ...
    */
    ...
    pub fn relay_message(&mut self, msg: &Message) -> Result<Option<Command>> {
        ...
        if targets.is_empty() {
            trace!("Empty targets while relaying...");
            return Ok(None);
        }
